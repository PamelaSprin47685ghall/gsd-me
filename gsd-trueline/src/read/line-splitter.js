...
// ==============================================================================
// Ported from trueline-mcp's line-splitter.ts to plain ESM JavaScript.
...
/** Buffer containing a CRLF pair (\r\n). */
export const CRLF_BUF = Buffer.from("\r\n");

/** Buffer containing a single CR byte (\r). */
 ...
 * @property {Buffer} lineBytes - Line content without EOL.
 * @property {Buffer} eolBytes - EOL bytes (LF_BUF | CRLF_BUF | CR_BUF | EMPTY_BUF).
 * @property {number} lineNumber - 1-based line number.
 */

/**
 * @typedef {Object} SplitChunksOpts
 * @property {boolean} [detectBinary] - When true, throws if a null byte (0x00) is encountered.
 */
 ...
 *
 * @param {AsyncIterable<Buffer>} chunks - An async iterable of byte buffers.
 * @param {SplitChunksOpts} [opts] - Options.
 * @returns {AsyncGenerator<RawLine>} An async generator of lines.
 */
export async function* splitChunks(chunks, opts = {}) {
   ...
   * @param {Buffer} [tail] - Optional tail buffer to append before flushing.
   * @returns {Buffer}
   */
  ...
  for await (const buf of chunks) {
    ...
    if (lineStart < buf.length) {
      const remainder = Buffer.from(buf.subarray(lineStart));
 ...
 * @param {{ detectBinary?: boolean }} [opts] - Options.
 * @returns {AsyncGenerator<RawLine>} An async generator of lines.
 */
export async function* splitLines(filePath, opts) {
