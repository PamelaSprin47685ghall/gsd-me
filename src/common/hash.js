...
// FNV-1a Hash
// ==============================================================================
// Ported from trueline-mcp's hash.ts to plain ESM JavaScript.
 ...
 *
 * @param {string} line - The string to hash.
 * @returns {number} 32-bit unsigned FNV-1a hash.
 */
export function fnv1aHash(line) {
 ...
 *
 * @param {Buffer} buf - The buffer containing bytes to hash.
 * @param {number} start - Start index (inclusive).
 * @param {number} end - End index (exclusive).
 * @returns {number} 32-bit unsigned FNV-1a hash.
 */
export function fnv1aHashBytes(buf, start, end) {
  let hash = FNV_OFFSET_BASIS;
 ...
 * in the reader.
 *
 * @param {number} accumulator - Current checksum accumulator.
 * @param {number} h - 32-bit line hash to fold in.
 * @returns {number} Updated accumulator.
 */
export function foldHash(accumulator, h) {
...
/**
 * Encode a 32-bit checksum as 6 lowercase letters via base-26.
 * 26^6 = 308M values, sufficient to distinguish checksums.
 *
 * @param {number} h - The 32-bit checksum value.
 * @returns {string} 6-character lowercase string.
 */
export function checksumToLetters(h) {
  let result = "";
  let n = h >>> 0;
  for (let i = 0; i < 6; i++) {
 ...
 * @param {number} hash - The checksum hash value.
 * @param {string} [startLetters] - Optional 2-letter start hash tag.
 * @param {string} [endLetters] - Optional 2-letter end hash tag.
 * @returns {string} Formatted checksum string.
 */
export function formatChecksum(startLine, endLine, hash, startLetters, endLetters) {
 ...
 */
export const LETTER_TABLE = /* @__PURE__ */ (() => {
  const t = new Array(676);
  for (let i = 0; i < 26; i++) {
    for (let j = 0; j < 26; j++) {
      t[i * 26 + j] = HASH_CHARS[i] + HASH_CHARS[j];
    }
  }
  return t;
})();
 ...
 *
 * XOR-folds upper and lower 16 bits to decorrelate the two characters.
 * Without this, FNV-1a's adjacent-byte correlation causes h%26 and
 * (h>>>8)%26 to cluster, using only ~50% of the 676-tag space.
 *
 * @param {number} h - The 32-bit hash value.
 * @returns {string} Two-character lowercase tag (e.g. "ab").
 */
export function hashToLetters(h) {
