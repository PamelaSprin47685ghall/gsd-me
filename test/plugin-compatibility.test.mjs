...
const createPluginHarness = ({ strictRegistration = false } = {}) => {
  ...
  const pi = {
    ...
    registerTool(tool) {
      if (strictRegistration && tools.has(tool.name)) throw new Error(`duplicate tool ${tool.name}`);
      tools.set(tool.name, tool);
    },
    registerCommand(name, config) {
      if (strictRegistration && commands.has(name)) throw new Error(`duplicate command ${name}`);
      commands.set(name, config);
    },
    registerShortcut(shortcut, config) {
      if (strictRegistration && shortcuts.has(shortcut)) throw new Error(`duplicate shortcut ${shortcut}`);
  ...
  return { pi, ctx, handlers, tools, commands, shortcuts, notifications, userMessages, customMessages, emitBeforeAgentStart, emitContext };
};
...
describe("all bundled plugins compatibility", () => {
  ...
  it("does not let Guardian recover from benign sibling plugin notifications", async () => {
    ...
    for (const handler of harness.handlers.get("notification") ?? []) {
      await handler({ severity: "warning", message: "magic-todo: session_start restored 0 todos and 0 backlog reports.", id: "n1" }, harness.ctx);
      await handler({ severity: "warning", message: "pruner: HINTS 加载警告 — missing file", id: "n2" }, harness.ctx);
      await handler({ severity: "warning", message: "[dag] M001/S01 {\"event\":\"tick\"}", id: "n3" }, harness.ctx);
    }
  ...
  it("keeps plugin registration and hooks idempotent when discovered twice", async () => {
    ...
    const hookCounts = new Map([...harness.handlers].map(([name, callbacks]) => [name, callbacks.length]));
    ...
    assert.deepEqual(new Map([...harness.handlers].map(([name, callbacks]) => [name, callbacks.length])), hookCounts);
